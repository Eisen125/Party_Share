...
public class FirebaseModel {
    ...
    FirebaseModel() {
        ...
        FirebaseFirestoreSettings settings = new FirebaseFirestoreSettings.Builder()
                ...
                .build();
        databasefirestore.setFirestoreSettings(settings);
        FS = FirebaseStorage.getInstance();
        autantichation = FirebaseAuth.getInstance();
    }


    public void getAllPosts(Model.Listener<List<Post>> callback) {
        databasefirestore.collection(Post.COLLECTION).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
    ...
    public void addPost(Post post, Model.Listener<Void> listener) {
        databasefirestore.collection(Post.COLLECTION).document(post.getId()).set(post.toJson())
    ...
    public void getPostById(String id, Model.Listener<Post> listener) {
        databasefirestore.collection(Post.COLLECTION).whereEqualTo(Post.ID, id).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                Post post = new Post();
                if (task.isSuccessful()) {
    ...
    void uploadImage(String name, Bitmap bitmap, Model.Listener<String> listener) {
        ...
        StorageReference imagesRef = storageRef.child("images/" + name + ".png");
    ...
    public void signUp(String email, String label, String password, Model.Listener<Pair<Boolean, String>> listener) {
        databasefirestore.collection(User.COLLECTION).whereEqualTo(User.ACCOUNT_LABEL, label).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    if (task.getResult().isEmpty()) {
                        autantichation.createUserWithEmailAndPassword(email, password)
                                .addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                                    ...
                                    public void onComplete(@NonNull Task<AuthResult> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            databasefirestore.collection(User.COLLECTION).add(user.toJson()).addOnCompleteListener(new OnCompleteListener<DocumentReference>() {
                                                @Override
                                            ...
                                            if (exception instanceof FirebaseAuthUserCollisionException) {
                                                ...
                                                listener.onComplete(new Pair<>(false, exception.getMessage()));
    ...
    public void login(String email, String password, Model.Listener<Pair<Boolean, String>> listener) {
        autantichation.signInWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    current_user = autantichation.getCurrentUser();
    ...
    public void getUserPosts(String label, Model.Listener<List<Post>> callback) {
        databasefirestore.collection(Post.COLLECTION)
